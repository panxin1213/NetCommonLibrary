...
{
    public class Image_Record_Service : ServiceBase<T_Image_Record>
    {
        ...
        public T_Image_Record GetByMd5(string md5)
        {
            var sql = "select * from T_Image_Record where F_Image_Record_Md5=@md5";

            return db.Query<T_Image_Record>(sql, new { md5 }).SingleOrDefault();
        }
        ...
        public bool UpdateNum(string filepath, int num, List<string> injson, IDbTransaction trans = null)
        {
            ...
            var m = db.Query<T_Image_Record>("select * from T_Image_Record where F_Image_Record_FilePath=@filepath", new { filepath }, trans).SingleOrDefault();
