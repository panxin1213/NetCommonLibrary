...
namespace 域名查询
{
    public partial class Form2 : Form
    {
        ...
        {
            _indexOfDomdain = 0;
            if (_indexOfDomdain >= listView1.Items.Count) return;
            var parms = new TaskMain2.TaskParms()
            {
                List = listView1,
                //LinkCount = Int32.Parse(comboBox1.Text),
                MaxTrend = Int32.Parse(comboBox1.Text),
                MaxWeight = Int32.Parse(comboBox2.Text),
            ...
            TaskMain2.index = _indexOfDomdain++;
            TaskMain2.threadCount = Int32.Parse(comboBox4.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            var c_trend = Int32.Parse(comboBox1.Text);
            var c_weight = Int32.Parse(comboBox2.Text);
            foreach (ListViewItem x in listView1.Items)
            {
                int trend, weight;
                if (!Int32.TryParse(x.SubItems[1].Text, out trend))
                    trend = -1;
                if (!Int32.TryParse(x.SubItems[2].Text, out weight))
                    weight = -1;
                var available = x.SubItems[3].Text.Equals("可注");
                var gfw = x.SubItems[4].Text;
                if (x.SubItems[1].Text == "是" || (trend >= c_trend && weight >= c_weight && (checkBox1.Checked ? available : true)))
                {
                    var d = new DomainModel()
                    {
                        ...
                        gfw = gfw,
                        available = available,
                        beian = x.SubItems[1].Text == "是"
                    };
                    list.Add(String.Format("{0},{1},{2},{3}", d.sogouTrend.ToString("D4"), d.sogouWeight.ToString("D2"), d.available ? 0 : 1,Guid.NewGuid().ToString()), d);
                }
            }
            ...
            if (saveFile1.ShowDialog() == System.Windows.Forms.DialogResult.OK && saveFile1.FileName.Length > 0)
            {
                ...
                try
                {
                    ...
                    sb.Append("</head><body>");
                    sb.Append("<table>");
                    sb.Append("<thead><th>域名</th><th>是否备案</th><th>搜狗年平均趋势</th><th>搜狗权重</th><th>能否注册</th><th>GFW</th></thead>");
                    for (int i = list.Count - 1; i >= 0; i--)
                    {
                        var x = list.Values[i];
                        sb.AppendFormat("<tr><td><a href=\"https://www.baidu.com/s?wd=site%3A{0}\" target=\"_blank\">{0}</a></td><td>{5}</td><td><a href=\"http://index.sogou.com/sidx?type=0&domain=1&query={0}\" target=\"_blank\">{1}</a></td><td><a href=\"http://www.baidu.com/s?wd=%22www.{0}%22&rn=100\" target=\"_blank\">{2}</a></td><td><a href=\"http://whois.chinaz.com/{0}\" target=\"_blank\">{3}</a></td><td><a target=\"_blank\" href=\"http://www.checkgfw.com/#www.{0}\">{4}</a></td></tr>", x.domain, x.sogouTrend, x.sogouWeight, x.available ? "可注" : "-", x.gfw, x.beian ? "是" : "否");
                    }
            ...
            //            br = br,
            //            available = available,
            //            beian = x.SubItems[4].Text.Equals("是")
            //        };
            ...
            //        sb.Append("</head><body>");
            //        sb.Append("<table>");
            //        sb.Append("<thead><th>域名</th><th>百度权重</th><th>外链域名数</th><th>外链数</th><th>备案</th><th>能否注册</th></thead>");
            //        for (int i = list.Count - 1; i >= 0; i--)
            //        {
            //            var x = list.Values[i];
            //            sb.AppendFormat("<tr><td>{0}</td><td>{1}</td><td><a href=\"http://www.baidu.com/s?wd=%22www.{0}%22&rn=100\" target=\"_blank\">{2}</a></td><td><a href=\"http://www.baidu.com/s?wd=%22www.{0}%22&rn=100\" target=\"_blank\">{3}</a></td><td><a href=\"http://icp.aizhan.com/{0}/\" target=\"_blank\">{4}</a><td><a href=\"http://whois.chinaz.com/{0}\" target=\"_blank\">{5}</a></td></tr>", x.domain, x.br, x.linkDomainCount, x.linkCount,x.beian?"已备":"-", x.available?"可注":"-");
            //        }
            //        sb.Append("</table></html>");
            //        File.WriteAllText(saveFile1.FileName,sb.ToString());
            //        Process.Start(saveFile1.FileName);
        ...
        private class DomainModel {
            ...
            //public int br = -1;
            public int sogouWeight = -1;
            public int sogouTrend = -1;
