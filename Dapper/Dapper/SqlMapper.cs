...
this IDbConnection cnn, string sql, dynamic param = null, IDbTransaction transaction = null, int? commandTimeout = null, CommandType? commandType = null
        ...
        private static IEnumerable<T> QueryInternal<T>(this IDbConnection cnn, string sql, object param, IDbTransaction transaction, int? commandTimeout, CommandType? commandType)
        {
            ...
            using (var cmd = SetupCommand(cnn, transaction, sql, info.ParamReader, param, commandTimeout, commandType))
            {
                ...
                try
                {
                    if (ConfigurationManager.AppSettings["ShowSql"] != null && Convert.ToBoolean(ConfigurationManager.AppSettings["ShowSql"]) && System.Web.HttpContext.Current.Request.IsLocal)
                    {
                        showsql = true;
                    }
                ...
                try
                {
                    if (showsql)
                    {
                        ...
                        var sqlprint = System.Web.HttpContext.Current.Items["SqlPrint"] as Dictionary<string, string>;

                        if (sqlprint == null)
                        {
                            sqlprint = new Dictionary<string, string>();
                        }

                        if (sqlprint.ContainsKey(cmd.CommandText))
                        {
                            sqlprint[cmd.CommandText] = _timer.ElapsedMilliseconds.ToString();
                        }
                        else
                        {
                            sqlprint.Add(cmd.CommandText, _timer.ElapsedMilliseconds.ToString());
                        }


                        System.Web.HttpContext.Current.Items["SqlPrint"] = sqlprint;
                    }
                }
                catch
                {
        ...
        static IEnumerable<TReturn> MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(this IDbConnection cnn, string sql, object map, object param, IDbTransaction transaction, string splitOn, int? commandTimeout, CommandType? commandType, IDataReader reader, Identity identity)
        {
            ...
            try
            {
                ...
                if (reader == null)
                {
                    ...
                    try
                    {
                        if (ownedCommand != null && ConfigurationManager.AppSettings["ShowSql"] != null && Convert.ToBoolean(ConfigurationManager.AppSettings["ShowSql"]) && System.Web.HttpContext.Current.Request.IsLocal)
                        {
                            showsql = true;
                        }
                ...
                if (mapIt != null)
                {
                    ...
                    try
                    {
                        if (showsql)
                        {
                            ...
                            if (sqlprint.ContainsKey(ownedCommand.CommandText))
                            {
                                sqlprint[ownedCommand.CommandText] = _timer.ElapsedMilliseconds.ToString();
                            }
                            else
                            {
                                sqlprint.Add(ownedCommand.CommandText, _timer.ElapsedMilliseconds.ToString());
                            }
